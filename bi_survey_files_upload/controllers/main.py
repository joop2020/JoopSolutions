    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        for question in questions:
            ...
            if question.question_type == 'upload_file':
                answer = post.get('all_attachment')
                if question.multipal_file == True and isinstance(answer, list):
                    all_dict = answer
                else:
...
class SurveyImageVideo(http.Controller):

    @http.route('/survey/attachment/add', type='http', website=True, auth='public', methods=['POST'])
    def attachment_add(self, name, file, res_id, **post):
    ...
    @http.route(['/survey/attachment/delete'], type='json', auth='public', website=True)
    def survey_attachment_delete(self, name, **kw):
        attachment = request.env['ir.attachment'].sudo().search([('id','=',name)])
        attachment.unlink()
        return True